...
### Merge-Konflikte
Wenn bei einem **Push**, einem **Pull** oder einem **Merge** ein Konflikt auftritt (heißt Änderungen online **und** lokal bzw. im aktuellen **und** dem zu mergenden Branch betreffen die gleichen Codeteile bzw. Zeilen), dann muss dieses erst bereinigt werden. Hierfür werden die betroffenen Dateien mit speziellen Bereichen versehen die aufzeigen wo sich die Konflikte befinden und auch welche Änderungen in den beiden Branches (oder zwischen Online und Lokal) sind. Die Bereiche sehen wie folgt aus:
```
<<<<<<< HEAD
  (lokale Änderungen - die Pfeile zeigen links raus aus der Datei)
=======
  (eingehende Änderungen - entweder von einem anderen Branch oder einem Online-Repository)
>>>>>>> branch-name (or commit-hash)
```
<sup>`HEAD` ist wie ein Zeiger auf den Commit der aktuellen ausgecheckt ist - in den meisten Fällen also der letzte Commit des aktuellen Branches. Das Git-Buch vergleicht dies mit einem Lesezeichen, das ja auch verschoben werden kann (wenn man ein früheres Commit auscheckt, also den Stand dieses Commits im Dateisystem wiederspiegelt).</sup>
Um diesen Konflikt zu lösen gilt es nun, sich entweder für einen Bereich von Änderungen zu entscheiden (und den anderen Bereich zu entfernen) oder sogar komplett umzuschreiben, sodass sich kein Konflikt mehr auftut. Wichtig ist nur, dass in jedem Fall auch die Marker entfernt werden, denn diese werden von *Git* geprüft und geben an, ob man sich um den Konflikt gekümmert hat. Im Beispiel oben löscht man also zumindest 4 Zeilen: Entweder die 3 Marker und die lokalen Änderungen oder die 3 Marker und die eingehenden Änderungen.
...
Bei einem Merge-Konflikt wird auch ein eigener *Commit* automatisch erstellt, das die Auflösung dessen dokumentiert - dies geschieht automatisch und dient dazu nachvollziehen zu können, weshalb Änderungen die eine Person gemacht hat nicht mehr vorhanden sind.
