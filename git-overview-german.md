...
### Rebase
Ein `Rebase` ist ähnliche zu einem `Merge`, bringt aber einen deutlichen Unterschied mit sich: Während ein Merge in einen eigenen Eintrag in der Geschichte des Repositories erhält (und demnach die bevorzugte Methode darstellt), ist es nicht immer möglich einen Konflikt mit einem **Merge** zu lösen - je mehr Änderungen sich zwischen den lokalen und den online gespeicherten Dateien befinden, umso schwieriger wird es diese zusammenzuführen. Hierfür gibt es dann ein **Rebase**, mit dem sich allerdings auch die Geschischte umschreibt und somit Daten zu Veränderungen verloren gehen.
Mit der Funktionsweise eines `Rebase` habe ich leider auch noch nicht viel Erfahrung und trete diesem Befehl mit Respekt gegenüber, weshalb ich dann meistens einfach eine Kopie meines lokalen Ordners erstelle, das Online-Repository neu klone und die Änderungen Schrittweise von der Kopie in das Repo übernehme. Gerade durch die Aussicht darauf dass Commits verloren gehen können (und ein Rebase bisher nur in Verbindung mit meinem Arbeitsplatz aufgetreten sind) bin ich diesbezüglich eher vorsichtig, finde aber dass man diesen Begriff zumindest mal grob gehört haben sollte.
